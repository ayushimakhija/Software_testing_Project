...
public class CustomerService {
    ...
    public List<Customer> listOfSubscribeCustomer(String customerId){
        List<Customer> customerList = new ArrayList<>();
        for(Customer cr : customerRepository.findAll()){
            if(cr.getEmailId().equals(customerId) && cr.isSubscribe() == true){
                customerList.add(cr);
            }
        }
        return customerList;
